...
banda(littoNebbia, argentina, 12345).
...
careta(personalFest).
careta(Festival):- festival(Festival, _, _), not(entradaVendida(Festival, campo)).
...
sobrevendido(Festival):-
    ...
    cantidadEntradasVendidas(Festival, CantidadVendidas),
    CantidadVendidas > Capacidad.

cantidadEntradasVendidas(Festival, CantidadVendidas):-
    findall(Entrada, entradaVendida(Festival, Entrada), ListaEntradas),
    length(ListaEntradas, CantidadVendidas).
...
recaudacionTotal(Festival, TotalRecaudado):-
    festival(Festival,_,Lugar),
    findall(Recaudado, (entradaVendida(Festival, Entrada), precio(Lugar, Entrada, Recaudado)), ListaResultante),
    sum_list(ListaResultante, TotalRecaudado).
    
precio(Lugar, campo, PrecioBase):- 
    lugar(Lugar, _, PrecioBase).
precio(Lugar, plateaGeneral(Zona), Recaudado):- 
    lugar(Lugar, _, PrecioBase), 
    plusZona(Lugar, Zona, Recargo), 
    Recaudado is PrecioBase + Recargo.
precio(Lugar, plateaNumerada(Numero), Recaudado):-
    lugar(Lugar, _, PrecioBase),
    Numero =< 10, Recaudado is PrecioBase * 6.
precio(Lugar, plateaNumerada(Numero), Recaudado):-
    lugar(Lugar, _, PrecioBase),
    Numero > 10 , Recaudado is PrecioBase * 3.
...
esMasPopularQue(UnaBanda, OtraBanda):-
    banda(UnaBanda,_, PopularidadBanda),
    banda(OtraBanda,_,PopularidadOtra),
    PopularidadBanda > PopularidadOtra.
